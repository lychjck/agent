...
**目标:** 把当前 ETF/基金持仓日报工具升级成一个本地个人持仓分析 agent：它能同步持仓、拉取行情、补全标的信息、做行业和资产分类、记住历史、按个人策略约束生成风险诊断和候选动作，并把所有建议交给用户确认，不自动交易。
...
- CLI 和核心分析逻辑：`/Users/liyanran/github/stock/stock_assistant.py`
...
- 后端测试：`/Users/liyanran/github/stock/tests/test_stock_assistant.py`
...
- `fetch_tzzb_holdings(config)` 当前返回 3 个值：`(holdings, source, summary)`。
- 但 `stock_assistant.py` 的 `run()` 里仍然按 2 个值接收。
- 位置：`/Users/liyanran/github/stock/stock_assistant.py:1385`。
...
| 策略约束工具 | 需要 | 读取个人风险偏好、仓位上限、配置目标 | 确定性 Python 函数 |
| 分类工具 | 需要 | 判断宽基、行业、主题、债券、海外、现金等 | 规则 + 缓存 + 搜索 + LLM |
| 组合分析工具 | 需要 | 计算集中度、分类占比、目标偏离、风险标签 | 确定性 Python 函数 |
| 历史记忆工具 | 需要 | 保存每日快照，对比昨天和今天变化 | 本地 JSON/JSONL |
| 候选动作工具 | 需要 | 根据规则生成可审阅动作 | 确定性 Python 函数 |
| LLM 报告工具 | 需要 | 解释证据、排序候选动作、生成报告 | LLM |
| 搜索工具 | 建议第二阶段加入 | 查询 ETF 跟踪指数、基金公司、行业归属、主题暴露 | 外部搜索 API 或手工 JSON |
...
10. 按你的策略配置检查风险。
11. 生成确定性的候选动作。
12. 把持仓、指标、分类、历史变化、风险、候选动作交给 LLM。
13. LLM 输出结构化 JSON。
14. 后端校验 JSON schema。
15. 如果 LLM 输出不合法，先触发一次 JSON 修复重试；仍失败才回退到规则引擎结果。
16. 保存快照。
17. 生成报告。
18. 触发告警。
19. 前端展示 agent 结果，等待你确认。
...
1. 先做 Task 10 的 SSE，避免第一版不可用。
2. 再做 Task 8 的 stable id，否则记忆系统会产生大量假变化。
3. 再做 Task 7 的定量候选动作，让 LLM 有明确解释对象。
4. 再做 Task 9 的结构化输出和修复重试。
5. 再补 Task 5/分类体系里的主动基金和搜索证据质量。
6. 已删除独立“本地分类引擎”任务；本地启发式只作为 Task 5 的最后 fallback。
...
query
  ...
  -> 交叉验证
  -> 置信度评分
  -> 写入 data/research/instruments/{code}.json
  -> 分类和风控只使用缓存后的结构化字段
```
  ...
  "sources": [
    {
      ...
      "fields_supported": ["tracked_index", "asset_class"]
    }
  ],
  "reviewed_by_user": false
}
...
```python
def classify_from_search_evidence(
    holding: Holding,
...
LLM 搜索归类不是直接“相信模型”。它只能把已筛选的搜索证据映射到受控 taxonomy，输出也要经过字段校验、来源校验和置信度规则。
...
- 修改：`/Users/liyanran/github/stock/stock_assistant.py`
- 新增：`/Users/liyanran/github/stock/tests/test_portfolio_policy.py`
...
- mock `fetch_bars`
- mock `call_llm`
- 验证完整 snapshot 包含分类、组合摘要、风险、候选动作、LLM 报告。
