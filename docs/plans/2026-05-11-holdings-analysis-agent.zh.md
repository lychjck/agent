# 我的持仓分析流水线与 LLM Agent 演进计划

> **给后续执行者:** 实现本计划时，请按任务逐项执行、逐项验证。不要跳过测试，不要一次性大重构。

**目标:** 第一版先把当前 ETF/基金持仓日报工具升级成一个本地个人持仓分析流水线：它能同步持仓、拉取行情、补全标的信息、做行业和资产分类、生成组合画像、记住历史，并在用户确认策略规则后生成风险诊断和候选动作。第一版不是严格意义上的 LLM-driven agent，而是“确定性分析流水线 + LLM 报告层”。真正由 LLM 规划、选择工具、调用工具、读取 observation 后继续推理的 Agent，放到后续独立里程碑。

**架构:** 第一版继续保持本地单体，不急着拆成多个服务。`stock_assistant/agent.py` 承载确定性流水线编排，`stock_assistant/agent_llm.py` 承载 LLM 报告输入/输出契约，`api.py` 对外提供 FastAPI 接口；`frontend/src/App.tsx` 展示持仓驾驶舱和 AI 报告。当前 API 路径里保留 `/api/agent/*` 命名只是历史兼容；概念上第一版应理解为分析流水线，不是 LLM 自主工具调用 Agent。
...
早期文档把第一版称为“Agent”，这个说法不够精确。后续阅读本计划时按下面区分：

- **持仓分析流水线 / Portfolio Analysis Pipeline**：后端按固定顺序调用 Python 函数，同步持仓、拉行情、分类、画像、生成历史 diff，最后把上下文交给 LLM 写报告。当前 Task 0-10 基本属于这一类。
- **LLM 报告层 / LLM Report Layer**：LLM 只消费已经准备好的事实上下文，输出组合诊断、单标的解释、观察条件和问题。LLM 不决定调用哪些工具。
- **LLM 工具调用 Agent / LLM Tool-Calling Agent**：LLM 根据任务目标自主规划下一步，选择安全工具，读取 observation 后继续推理，循环直到完成。这才是用户理想中的 Agent，放在后续 Milestone 8。

因此，本文中已有的 `agent` 字段、`/api/agent/*` 路由和 `stock_assistant/agent.py` 文件名是历史命名和兼容接口；第一版产品描述应称为“持仓分析流水线”。只有 Milestone 8 才使用严格意义上的 LLM Agent 定义。
...
- Task 5+：**天天基金 (eastmoney) 直接 API 数据获取与规则分类**（新增 `stock_assistant/eastmoney.py`）。
  - 绕过搜索引擎无法解析 JS 渲染页面的问题，直接调用 `pingzhongdata/{code}.js` 和 `FundArchivesDatas.aspx` 获取基金名称、持仓股票、仓位比例等结构化数据。
  - 分类流水线升级为三层架构：天天基金结构化 API → 规则引擎分类 → LLM 验证/修正，搜索引擎仅作为最后 fallback。
  - 已验证 011068（华宝资源优选混合C）：规则分类 asset_class=mixed_allocation, sector=materials, confidence=0.90，前十大重仓股（紫金矿业、藏格矿业、洛阳钼业等）全部正确解析。
- Task 6：组合画像和事实观察项。
...
### 2.1 第一版不是严格 LLM Agent

第一版不是严格意义上的 LLM-driven agent。它是确定性流水线：Python 后端决定工具调用顺序，LLM 只在最后消费上下文并生成解释。

LLM 适合做解释、归纳、排序、生成用户可读报告，但在第一版中不承担工具规划、事实获取、分类缓存、组合计算、风控阈值和状态管理。
...
| LLM 报告工具 | 需要 | 解释证据、解释组合画像、必要时排序候选动作、生成报告 | LLM 报告层 |
| 搜索工具 | 已完成基础版 | 查询 ETF 跟踪指数、基金公司、行业归属、主题暴露 | Tavily / Brave / 手工 JSON |
| 告警工具 | 第三阶段加入 | 风险触发后生成提醒 | 本地报告，后续可接飞书/邮件 |
| 交易工具 | 不做 | 自动下单、提交交易 | 第一版禁止 |

这里的“工具”不是 OpenAI tool calling、MCP server 或插件。第一版直接实现为稳定的 Python 函数，每个函数有明确输入输出，由后端流水线显式调用。等本地逻辑稳定后，再决定是否把部分函数暴露给 LLM tool calling 或 MCP。

严格意义上的 LLM Agent 应该具备：

1. LLM 根据用户目标决定下一步要查什么。
2. LLM 选择一个受控工具并给出参数。
3. 后端校验权限和参数，执行工具，返回 observation。
4. LLM 基于 observation 继续分析，必要时再次调用工具。
5. 信息足够后，LLM 输出最终报告。

这套循环不属于第一版，放到后续 Milestone 8。
...
你的判断是对的：如果后续要做真正的 LLM 工具调用 Agent，尤其涉及行业分类、ETF 跟踪指数、基金主题、发行方、跨市场资产类型，搜索工具一定会有用。第一版流水线也需要搜索能力，但搜索触发由后端流程和用户刷新动作控制，不由 LLM 自主决定。
...
3. 再做 Task 10 的 SSE，避免第一版分析流水线运行时间过长导致 API 超时。
...
### Task 6：组合画像和观察项（已完成基础版，暂不做策略检查）
...
- 已新增 `stock_assistant/portfolio.py`。
- 已实现 `value_map_to_pct()`、`summarize_portfolio()`、`generate_portfolio_observations()`。
- 已从 `stock_assistant/__init__.py` 导出 Task 6 函数。
- 已新增 CLI 子命令：`uv run python stock_assistant.py --config config.toml profile`。
- `profile --holdings <csv/xlsx>` 支持跳过账本同步，直接对指定持仓文件生成画像 JSON。
- `profile` 默认只读 `config` 和已有分类缓存，不触发搜索或分类 LLM。
- 只有显式使用 `--refresh-classification` 时，才允许补全分类并可能触发搜索/分类 LLM。
- 分类刷新日志已补充标的、触发原因、缓存状态、证据数量、证据评分、搜索来源和 LLM 配置摘要。
- 已新增 FastAPI 接口：`GET /api/profile`。
- 已接入前端组合画像工作区，展示资产大类、策略、行业、Top 持仓、观察项和分类明细。
- 已新增 `tests/test_portfolio_profile.py`。
- 已通过 `uv run python -m unittest discover -s tests`，当前 36 个测试通过。
...
### Task 9：LLM 报告交互层和结构化校验

**涉及文件：**

- 新增：`/Users/liyanran/github/stock/stock_assistant/agent_llm.py`
- 修改：`/Users/liyanran/github/stock/stock_assistant/llm.py`
- 修改：`/Users/liyanran/github/stock/api.py`
- 修改：`/Users/liyanran/github/stock/stock_assistant/__init__.py`
...
Task 9 不只是修 JSON，而是补齐 agent 和 LLM 之间的正式 contract。前 8 个任务产出的持仓、技术指标、分类、组合画像、观察项、历史变化和候选动作，必须先被整理成一份可审计、可脱敏、可校验的 LLM 输入；LLM 再基于这份输入做解释、诊断、动作审阅和问题生成。

LLM 的角色：

- 解释组合结构、暴露、趋势、分类可信度和历史变化。
- 审阅规则引擎已经生成的候选动作。
- 在缺少用户策略规则时提出问题和观察条件。
- 不直接承担事实获取、仓位计算、分类缓存、风控阈值和交易动作生成。
...
发送给 LLM 的 `holdings` 字段只保留：

- `code` / `name` / `asset_type`
- `quantity` / `cost_price` / `market_value` / `profit_pct` / `hold_profit` / `day_profit`
- `classification`
- `technical`
- `evidence_refs`
  ...
  "holding_analysis": [
    {
      ...
      "evidence_refs": ["holding:510300:technical", "holding:510300:classification"]
    }
  ],
  "action_reviews": [
    {
...
- `risk_tags`
- `action_items`，由 `holding_analysis` 和 `action_reviews` 派生，确保每个标的卡片都有单标的建议。
- `detailed_analysis`

历史快照上下文去重：

- `diff_agent_snapshots()` 必须基于事实字段生成 fingerprint，忽略 `generated_at`、`agent_report`、`model`。
- 如果最新快照和当前快照的事实字段一致，`history_diff` 只返回 `duplicate_of_latest=true` 和 `portfolio_changes.unchanged=true`。
- 重复事实快照不再把 `continued` 风险/动作噪音塞给 LLM。
- 保存快照前也要比较事实 fingerprint；相同则跳过保存，避免收盘后同一批数据被反复落盘。

结构化输出优先级：

1. 上策：如果当前 LLM provider 支持 JSON Mode，且 `[llm] supports_response_format = true`，请求里带 `response_format={"type":"json_object"}`。
...
def llm_structured_kwargs(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if mode == "auto" and config_bool(config.get("llm", {}).get("supports_response_format", False)):
        return {"response_format": {"type": "json_object"}}
    return {}
```

注意：ModelScope、EasyRouter 或其他 OpenAI-compatible provider 不一定完整支持 `response_format`、JSON Schema 或 Tool Calling。`auto` 模式必须受 provider capability 开关控制，不能假设所有 provider 都兼容。
...
- 可以解释风险。
- 必须输出 JSON。
- 必须为每个输入 `holdings` 输出一条 `holding_analysis`；这是单标的解释型建议，不是直接交易指令。
- 每条诊断、动作审阅、观察条件和问题都应尽量引用 `evidence_refs`。

测试：

- LLM context 包含 portfolio / holdings / observations / classifications / history_diff。
- LLM context 不包含 Cookie / API key / source_row / 原始 raw_content / account id。
- 合法 JSON 能解析。
...
- 不存在的 `evidence_ref` 会被过滤。
- LLM 漏掉单标的建议时，后端用技术规则为缺失标的补 `holding_analysis`。
- 后端会派生 `risk_tags` / `action_items` / `detailed_analysis` 兼容旧前端。
- 相同事实快照会被识别为 `duplicate_of_latest`，不会生成冗余历史上下文，也不会重复保存。
...
- 新增：`/Users/liyanran/github/stock/stock_assistant/agent.py`
- 修改：`/Users/liyanran/github/stock/api.py`
- 新增：`/Users/liyanran/github/stock/tests/test_agent_orchestrator.py`

实现原则：

- 第一版 orchestrator 是确定性分析流水线，直接调用 Python tools。
- 不做 MCP server。
- 不让 LLM 自主决定调用哪些工具。
...
命名说明：代码和 API 仍保留 `agent` 命名，原因是兼容前端和历史计划；但本任务实现的是固定流水线编排，不是 LLM 工具调用 Agent。
...
- 兼容保留：`POST /api/analyze`，内部复用 agent 事件流并映射成旧前端使用的数字 step。
...
第一步先加类型。类型名可以沿用现有 `Agent*` 命名以减少代码迁移成本，但产品语义应理解为“分析结果”，不是自治 Agent 的内部状态：
...
后端 SSE 已在 Task 10 中作为第一版主接口实现。本任务只负责前端消费和展示。接口路径可以继续使用 `/api/agent/run/stream`，但这里的 `agent` 是历史兼容命名，实际事件来自固定分析流水线。
...
6. 系统自主性：只生成建议，还是也维护一份“目标配置草案”？ 维护一份“目标配置草案”
...
```bash
cd /Users/liyanran/github/stock
uv run python stock_assistant.py --config config.toml profile
uv run python stock_assistant.py --config config.toml profile --holdings tests/fixtures/holdings.csv
```

默认 `profile` 只读取已有分类配置和缓存，不触发搜索或分类 LLM。如果需要重新补全分类：

```bash
uv run python stock_assistant.py --config config.toml profile --refresh-classification
```

保存组合画像 JSON：

```bash
uv run python stock_assistant.py --config config.toml profile > reports/portfolio-profile.json
```
...
页面打开后会自动请求 `GET /api/profile`，默认不触发搜索或分类 LLM；点击“刷新分类画像”才会请求 `GET /api/profile?refresh_classification=true`。
...
- 有快照时返回最新分析快照；接口字段名可以继续保持 `agent snapshot` 兼容。
...
- 告警可用。
- 前端展示 LLM 报告、风险、动作、问题、历史变化。
- README 解释第一版是分析流水线而非严格 LLM Agent，并说明搜索、隐私、分类和风险边界。

---

### Milestone 8：真正的 LLM 工具调用 Agent（后续独立阶段）

详细实施计划见：`/Users/liyanran/github/stock/docs/plans/2026-05-12-llm-tool-agent-milestone8.zh.md`。

范围：

- 新增只读 `tool_registry`，把安全工具暴露给 LLM 调用。
- 新增 `agent_loop`，让 LLM 以“计划 → 工具调用 → observation → 继续计划”的循环完成任务。
- 新增 `tool_executor`，统一做参数校验、权限控制、超时和错误包装。
- 新增 `agent_trace`，保存每轮 LLM tool call、observation 和最终回答，方便回放和审计。
- 第一版只允许只读工具，不允许交易、写配置、读 Cookie、读 API key、读原始 TZZB 响应。

候选工具：

| 工具 | 作用 | 敏感级别 | 第一批是否允许 |
| --- | --- | --- | --- |
| `get_current_holdings` | 返回脱敏后的当前持仓 | 中 | 是 |
| `get_portfolio_profile` | 返回组合画像 | 低 | 是 |
| `get_holding_technical` | 返回单标的技术指标 | 低 | 是 |
| `get_classification` | 返回标的分类和置信度 | 低 | 是 |
| `search_instrument_metadata` | 搜索标的元数据 | 中 | 可选 |
| `load_snapshot_summary` | 读取历史快照摘要 | 低 | 是 |
| `compare_snapshots` | 对比两个快照 | 低 | 是 |
| `generate_candidate_actions` | 基于已确认 policy 生成动作草案 | 中 | 后置 |

Agent loop 草案：

```text
user_goal
  -> LLM: decide next tool
  -> backend: validate tool call
  -> tool: execute
  -> observation
  -> LLM: decide continue or finish
  -> final_report
```

退出条件：

- LLM 至少能自主选择 2 个以上工具完成一次“分析某个 ETF 要不要处理”的任务。
- 每次工具调用都有 trace。
- 工具调用参数经过后端白名单校验。
- LLM 无法读取 Cookie、API key、原始账户响应。
- LLM 不能执行交易或写入策略；候选动作仍必须由规则工具生成或用户确认。
- 有测试覆盖：正常多轮工具调用、非法工具拒绝、非法参数拒绝、工具失败后的恢复。

和第一版流水线的关系：

- 第一版流水线继续作为稳定 fallback。
- LLM Agent 可以复用第一版已有工具函数，不重写行情、分类、画像和快照逻辑。
- 前端可以先隐藏 LLM Agent 入口，只在开发/调试模式暴露。
...
先把本地个人持仓分析流水线做稳，再考虑真正的 LLM 工具调用 Agent。
