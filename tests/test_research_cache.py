...
class TestResearchCache(unittest.TestCase):
    ...
    def test_low_confidence_unreviewed_cache_is_ignored_when_threshold_configured(self):
        h = Holding(code="512880", name="证券ETF")
        cls = InstrumentClassification(
            code="512880",
            name="证券ETF",
        ...
        self.assertIsNone(load_cached_classification(h, cfg))
    ...
    def test_manual_json_search(self):
    ...
    def test_manual_json_search_with_llm_classifier(self):
        cfg = dict(self.config)
        cfg["classification"] = dict(cfg["classification"])
